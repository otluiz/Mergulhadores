...
"esta função mete um 'M' na coordenada (x y) à direita "
  (setq tab (car no))
  (setq x (first (caddr no)))
  (setq y (second (caddr no)))
  (cond
    ((null tab) nil)
    ((and (coord-valida? (1+ x) y tab) (posso-mover? x y tab))
       (mergulhador
	(setf (poe-a x y (poe-mergulhador (1+ x) y tab)) tab)
	(cadr no) ;1 id padrão	
	(caddr no) ;; não faz nada com a origem
        (push (cadddr no);; list (1+ x) y) ;; posição atual
        (cons (list (1+ x) y) (list (third no) (fifth no)));; posição antes
...
(defun esquerda (no &optional(tab (car no)) (x (first (caddr no))) (y (second (caddr no))))
...
(defun acima (no &optional(tab (car no)) (x (first (caddr no))) (y (second (caddr no))))
  ...
  (cond
    ...
    ((and (coord-valida? x (1+ y) tab) (posso-mover? x y tab))
     (mergulhador
      ...
      (cadr no)
      (caddr no) ; não faz nada com a origem
      (list x (1+ y)) ; posição atual
      (cons (list x (1+ y)) (list (third no) (fifth no))) ;; posição antes
      (sixth no)
      )
     )
    (t nil)
...
(defun abaixo (no &optional(tab (car no)) (x (first (caddr no))) (y (second (caddr no))))
  ...
  (cond
    ...
    ((and (coord-valida? x (1- y) tab) (posso-mover? x y tab))
     (mergulhador
      ...
      (list x (1- y)) ;4º (fourth no));; posição atual
      (cons (list x (1- y)) (list (third no))) ;; posição antes
      (sixth no)
      )
     )
   (t nil)
