  ...
  (let ((ncol (length (car tab))))
    (cond
     ...
     ((and (< col ncol) (coord-valida? (+ x val) y tab))
      (direita x y (poe-a col y (poe-mergulhador (+ val x) y tab)) (1- val) (1+ col))) ;; anda + um passo
     (t nil))
    )
  )
  ...
  (let ((ncol (length (car tab))))
    (cond
     ...
     ((and (coord-valida? (- x val) y tab)) ;;(posso-mover? x y tab))
      (esquerda x y (poe-a col y (poe-mergulhador (if (> x 3) (- x val) (- ncol x val)) y tab))(1- val) (1- col)))
     (t nil))
    )
  )
...
(defun abaixo (x y tab &optional (val 1) (lin y) (pos-lin (- y val)))
  ...
  (cond
   ...
   ((and (coord-valida? x (- y val) tab) (posso-mover? x y tab))
    (abaixo x y (poe-a x lin (poe-mergulhador x (- y val) tab)) val (1- lin) (1- pos-lin)))
   (t nil))
  )
...
(defun left-rotate(lista);; espiral esquerda => sestra
  "move o 1º elemento para o fim da lista"
  (append (rest lista) (list (first lista)))
  )

;;; roda uma lista de argumentos, para a direita
(defun right-rotate(lista);; espiral direita => destra
  "move o ultimo elemento para a frete da lista"
