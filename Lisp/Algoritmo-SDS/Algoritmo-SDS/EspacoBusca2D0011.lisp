...
" devolve uma linha de uma lista, conforme um indice invertido"
  (cond
    ((null tab) (car tab))
...
;;; recebe um no e devolve um no com os movimentos aplicados
;;; mergulhador : um mergulhador => um nó
;;; n-giros : numero de voltas ao entorno da origem
;;; cvoltas : um contador de voltas
(defun destra (no &optional (n-giros 1) (tab (car no)))
  (let ((lst-jog '(direita abaixo esquerda acima))
        ;(c-voltas (coord-espiral nvoltas))
        (x (caar (coord-merg tab)))
	(y (cadar(coord-merg tab)))
        )
    (cond
     ((< n-giros 1) no)
     (t
      (destra
       (push 
        (list
         (setf tab (funcall (car (right-rotate lst-jog)) x y tab))
         (cadr no);;; id do mergulhador
         (caddr no);; origem do movimento
         (list (setf x (caar (coord-merg tab))) (setf y (caar (coord-merg tab))));(cadddr no)); posicao atual
         (cons (cadddr no) (fifth no));; posicao anterior
         (sixth no);; funcao de avaliacao
        ) no) (1- n-giros) tab))
     )
    )
  )
...
(defun mergulhador (tab &optional
                    (espaco tab)
		    (mergul-id 1)
		    (origem '(3 3))
		    (pos-atual(coord-merg tab))
		    (pos-antes nil)
		    (f-aval 0)
		    )
  (list 
...
;; função objetivo
(defun f-obj (no &optional (tab (car no)) (lst (car tab)) (col (length (car tab))) (lin 1))
  "Recebe um espaço de busca e devolve uma lista com as coordenadas dos objetivos "
  (let ((nlin (length tab)) (ncol (length (car tab))))
    (cond
     ((> lin nlin) nil)
     ((= col 0) (f-obj no tab lst (+ col ncol) (1+ lin)))
     ((eql (devol-ele col (devol-lin lin tab)) 0) ;; se encontra um objetivo, mover a origem para + proximo
      (cons (list 't-> col lin) (f-obj no tab (cdr lst) (1- col) lin)))
     (t (f-obj no tab (cdr lst) (1- col) lin))
     )
    )
  )


;(defun rotate(lista no n &optional (lst-c (coord-espiral n)))
;  "move o ultimo elemento para a frente da lista"
;  (labels ((aux-rotate (fn x y tab n lst-c)
;                         (if (= 0 n) tab
;                           (aux-rotate (funcall (car fn) x y tab) x y tab (1- n) (cdr lst-c))))
;           )
;    (aux-rotate (right-rotate lista) (car lst-c) (car lst-c) tab n lst-c))
;  )
...
(defun procura-mergul (lin &optional (val 0))
  "verifica se existe '1' numa linha"
  (cond
    ((null lin) val)
    ((eql (car lin) '1) (procura-mergul (cdr lin) (1+ val)))'
